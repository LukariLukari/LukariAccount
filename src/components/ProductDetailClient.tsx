...
export default function ProductDetailClient({ product }: ProductDetailClientProps) {
  ...
  return (
    <div className="min-h-screen bg-asphalt text-paper font-sans selection:bg-paper selection:text-asphalt">
      <main className="max-w-[1180px] mx-auto px-5 lg:px-6 py-5">
        <Link
          href="/products"
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-start">
          <motion.div
            ...
            className="flex-[1.55] bg-paper/5 backdrop-blur-3xl rounded-[1.75rem] p-4 lg:p-5 relative overflow-hidden shadow-2xl border border-paper/10 flex flex-col justify-center"
          >
            ...
            <div className="relative z-10 flex flex-col gap-4 h-full">
              <motion.div
                ...
                className="w-full h-[320px] lg:h-[360px] relative rounded-[1.35rem] overflow-hidden shadow-2xl group shrink-0 border border-paper/10"
              >
                {product.image ? (
                  <Image
                    src={product.image}
              ...
              <div className="w-full flex flex-col justify-center">
                ...
                >
                  ...
                  <h1
                    className="text-[1.35rem] lg:text-[1.75rem] font-montserrat font-bold tracking-tight mb-2 leading-none text-paper uppercase whitespace-nowrap truncate max-w-full"
                    title={product.name}
                  >
                    {product.name}
                  </h1>
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_auto] gap-4 xl:items-end">
                    ...
                    <div className="flex flex-wrap items-center gap-2.5 xl:justify-end">
                      ...
                      <motion.button
                        ...
                        className="px-5 py-2.5 rounded-full !bg-[#efede3] !text-[#302f2c] font-montserrat font-bold text-[9px] uppercase tracking-[0.16em] hover:scale-[1.02] active:scale-[0.98] transition-all shadow-2xl"
                      >
                        Thanh toán ngay
                      </motion.button>
          ...
          {(warrantyItems.length > 0 || features.length > 0 || instructions.length > 0) && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.3, ease: "easeOut", delay: 0.1 }}
              className="flex-[0.7] bg-paper/5 backdrop-blur-3xl rounded-[1.75rem] p-4 lg:p-5 relative overflow-hidden shadow-2xl border border-paper/10 flex flex-col gap-5"
            >
              ...
              {instructions.length > 0 && (
                <div>
                  ...
                  <ol className="space-y-2.5 text-[13px] text-paper">
                    {instructions.map((instruction, index) => (
                      <li key={`${instruction}-${index}`} className="flex gap-3">
                        <span className="w-5 h-5 rounded-full bg-paper/5 border border-paper/10 flex items-center justify-center text-[9px] font-bold text-[#FF8C00] shrink-0">
                          {index + 1}
                        </span>
        ...
        {detailParagraphs.length > 0 && (
          <section className="mt-5 bg-paper/5 backdrop-blur-3xl rounded-[1.75rem] p-4 lg:p-5 border border-paper/10 shadow-2xl">
            <div className="flex flex-col lg:flex-row lg:items-start gap-5">
              <div className="lg:w-52 shrink-0">
                <p className="text-[10px] font-montserrat font-bold uppercase tracking-[0.24em] text-[#FF8C00] mb-3">
                  Mô tả
                </p>
                <h2 className="text-base font-montserrat font-bold tracking-[0.18em] uppercase text-paper/80">
                  Thông tin chi tiết
                </h2>
              </div>
              <div className="flex-1 space-y-3">
                {detailParagraphs.map((paragraph, index) => (
                  <p key={`${paragraph}-${index}`} className="text-paper text-[13px] leading-relaxed">
