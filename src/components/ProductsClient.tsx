...
export default function ProductsClient({ initialProducts, initialQuery = "" }: ProductsClientProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result;
  }, [initialProducts, searchQuery, selectedCategories, sortBy]);
    ...
    <main className="min-h-screen bg-asphalt text-paper">
      <div className="max-w-[1440px] mx-auto px-4 md:px-10 pb-16 md:pb-20">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="flex gap-3 overflow-x-auto pb-1 md:overflow-visible md:pb-0">
            ...
            <button
              ...
              className={`flex shrink-0 items-center gap-2 px-5 py-3.5 border rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${
              ...
              <div className="bg-paper/5 border border-paper/10 rounded-2xl p-6">
                ...
                <div className="flex flex-wrap gap-3">
                  {categories.map((cat) => {
                    ...
                    const count = initialProducts.filter((p) => p.category.toLowerCase() === cat.toLowerCase()).length;
                    return (
                      <button
                        key={cat}
                      ...
                      >
                        <span>{cat}</span>
                        <span className={`text-[8px] px-1.5 py-0.5 rounded-full ${isSelected ? "bg-asphalt/10 text-asphalt/60" : "bg-paper/5 text-paper/20"}`}>
                          {count}
                        </span>
                      </button>
                    );
        ...
        {filteredProducts.length > 0 ? (
          <motion.div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
