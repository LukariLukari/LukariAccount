...
import { Search, Menu, ShoppingBag, X, ArrowRight, Wallet } from "lucide-react";
          ...
          <div className="flex items-center gap-1 sm:gap-6 shrink-0">
            <div className="hidden lg:flex items-center gap-6 mr-2">
              ...
              {!isAdminRoute && (
                <Link
                  href="/profile"
                  className="inline-flex items-center gap-2 rounded-full border border-[#FF8C00]/25 bg-[#FF8C00]/15 px-4 py-2 text-[10px] font-montserrat font-bold uppercase tracking-widest text-[#FFB45C] shadow-[0_10px_30px_rgba(255,140,0,0.12)] transition-all hover:scale-105 hover:bg-[#FF8C00] hover:text-asphalt"
                >
              ...
              <div className="bg-[#1a1a1a] border border-paper/10 rounded-[2.5rem] shadow-2xl p-8 space-y-8">
                <div className="space-y-4">
                  ...
                  <div className={isAdminRoute ? "grid grid-cols-1 gap-2.5" : "grid grid-cols-2 gap-4"}>
                    {(isAdminRoute ? adminMenuItems : [{label: 'Nạp tiền', href: '/profile'}, {label: 'Products', href: '/products'}, {label: 'Tài nguyên', href: '/resources'}, {label: 'Cart', href: '/cart'}, {label: 'Profile', href: '/profile'}]).map((item) => (
                      <Link 
                        key={item.label}
                        href={item.href}
