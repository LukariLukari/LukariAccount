...
export default function CartPage() {
  ...
  const requestCheckout = async (
    createOrder: boolean,
    note = "",
    paymentMethod: "bank" | "wallet" = "bank"
  ) => {
      ...
      {cart.length > 0 && (
        <PaymentPopup
          ...
          onSubmitOrder={async (note) => {
            const summary = await requestCheckout(true, note, "bank");
            setCheckoutSummary(summary);
            return { orderCode: summary.order?.orderCode };
          }}
          onSubmitWalletOrder={async (note) => {
            const summary = await requestCheckout(true, note, "wallet");
            setCheckoutSummary(summary);
            clearCart();
            return { orderCode: summary.order?.orderCode };
          }}
